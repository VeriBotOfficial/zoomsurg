...
class Patient:
    ...
    def UseTool(self, toolType: Enum) -> bool: #Use the selected tool based on the tool type
        ...
        match toolType: 
            case ToolType.SurgicalAntiseptic:
                if NurseChance:
                    self.NurseText = TextManager.PositiveText("[Nano Nurse Bot]: ") + TextManager.SoftText("Nano nurse bot prevented you from losing one " + TextManager.PurpieText("antiseptic"))
                else:
            ...
            case ToolType.SurgicalDefib:
                if NurseChance:
                    self.NurseText = TextManager.PositiveText("[Nano Nurse Bot]: ") + TextManager.SoftText("Nano nurse bot prevented you from losing one " + TextManager.PurpieText("defibrillator"))
                else:
                    self.DefibCount += 1
                    self.NurseText = ""
            ...
            case ToolType.SurgicalSponge:
                if NurseChance:
                    self.NurseText = TextManager.PositiveText("[Nano Nurse Bot]: ") + TextManager.SoftText("Nano nurse bot prevented you from losing one " + TextManager.PurpieText("sponge"))
                else:
                    self.SpongeCount += 1
                    self.NurseText = ""
            ...
            case ToolType.SurgicalScalpel:
                if NurseChance:
                    self.NurseText = TextManager.PositiveText("[Nano Nurse Bot]: ") + TextManager.SoftText("Nano nurse bot prevented you from losing one " + TextManager.PurpieText("scalpel"))
                else:
                    self.ScalpCount += 1
                    self.NurseText = ""
            ...
            case ToolType.SurgicalStitches:
                if NurseChance:
                    self.NurseText = TextManager.PositiveText("[Nano Nurse Bot]: ") + TextManager.SoftText("Nano nurse bot prevented you from losing a " + TextManager.PurpieText("stitch"))
                else:
                    self.StitCount += 1
                    self.NurseText = ""
            ...
            case ToolType.SurgicalUltrasound:
                if NurseChance:
                    self.NurseText = TextManager.PositiveText("[Nano Nurse Bot]: ") + TextManager.SoftText("Nano nurse bot prevented you from losing a " + TextManager.PurpieText("ultrasound"))
                else:
            ...
            case ToolType.SurgicalLabKit:
                if NurseChance:
                    self.NurseText = TextManager.PositiveText("[Nano Nurse Bot]: ") + TextManager.SoftText("Nano nurse bot prevented you from losing a " + TextManager.PurpieText("Lab Kit"))
                else:
                    self.LabKitCount += 1
                    self.NurseText = ""
            ...
            case ToolType.SurgicalAntibiotics:
                if NurseChance:
                    self.NurseText = TextManager.PositiveText("[Nano Nurse Bot]: ") + TextManager.SoftText("Nano nurse bot prevented you from losing a " + TextManager.PurpieText("Antibiotics"))
                else:
            ...
            case ToolType.SurgicalSplint:
                if NurseChance:
                    self.NurseText = TextManager.PositiveText("[Nano Nurse Bot]: ") + TextManager.SoftText("Nano nurse bot prevented you from losing a " + TextManager.PurpieText("Splint"))
                else:
                    self.SplintCount += 1
                    self.NurseText = ""
            ...
            case ToolType.SurgicalPins:
                if NurseChance:
                    self.NurseText = TextManager.PositiveText("[Nano Nurse Bot]: ") + TextManager.SoftText("Nano nurse bot prevented you from losing a " + TextManager.PurpieText("Pins"))
                else:
                    self.PinCount += 1
                    self.NurseText = ""
            ...
            case ToolType.SurgicalAnesthetic:
                if NurseChance:
                    self.NurseText = TextManager.PositiveText("[Nano Nurse Bot]: ") + TextManager.SoftText("Nano nurse bot prevented you from losing a " + TextManager.PurpieText("Anesthetic"))
                else:
                    self.AnestCount += 1
                    self.NurseText = ""
            ...
            case ToolType.SurgicalTransfusion:
                if NurseChance:
                    self.NurseText = TextManager.PositiveText("[Nano Nurse Bot]: ") + TextManager.SoftText("Nano nurse bot prevented you from losing a " + TextManager.PurpieText("Transfusion"))
                else:
            ...
            case ToolType.SurgicalClamp:
                if NurseChance:
                    self.NurseText = TextManager.PositiveText("[Nano Nurse Bot]: ") + TextManager.SoftText("Nano nurse bot prevented you from losing a " + TextManager.PurpieText("Clamp"))
                else:
                    self.ClampCount += 1
                    self.NurseText = ""
    ...
    def UpdatePatientUITexts(self): #The function that sets UI elements texts
        ...
        if self.TrainE:
            ...
            # Heart Stopped
            if self.HeartDamage > 0:
                self.TrainEText += TextManager.ErrorText("Heart Stopped") + \
                                   f" - You need to {TextManager.WarningText('Revive')} your patient with a {TextManager.PurpieText('Defiblirator')}!"
                return

            # Awake
            if self.SleepLevel == 0 and self.Incisions > 0:
                self.TrainEText += TextManager.ErrorText("Awake") + \
                                   f" - Your patient is {TextManager.WarningText('Awake')}. Use {TextManager.PurpieText('Anesthetic')} to put them to sleep until you have closed the wound.\n"

            # Stitch
            if self.Incisions > 0 and self.IsPatientFixed:
                self.TrainEText += TextManager.PositiveText("Stitch it Up!") + \
                                   f" - The issue is fixed! It's time to close it up with {TextManager.PurpieText('Stitches')}.\n"

            # FixIt
            if self.IsFixable:
                self.TrainEText += TextManager.PositiveText("Fix It!") + \
                                   f" - You have found the issue and can now {TextManager.PurpieText('Fix It')}.\n"

            # Cleanless
            if self.SiteSanitation < 1:
                self.TrainEText += TextManager.PositiveText("Clean the Area") + \
                                   f" - Clean the area with {TextManager.PurpieText('Antiseptic')}.\n"

            # PrepPatient
            if self.SleepLevel == 0 and self.IsUltrasoundUsed and self.IncisionsNeeded > 0:
                self.TrainEText += TextManager.PositiveText("Prep Patient") + \
                                   f" - Apply {TextManager.PurpieText('Anesthetic')} to put the patient to sleep.\n"

            # Incision
            if not self.IsFixable and not self.IsPatientFixed and self.SleepLevel > 0 and self.IsUltrasoundUsed:
                self.TrainEText += TextManager.PositiveText("Make an Incision!") + \
                                   f" - Use {TextManager.PurpieText('Scalpel')} to make an incision.\n"

            # Visibility
            if self.SiteDirtyness >= 4:
                self.TrainEText += TextManager.WarningText("Poor Visibility") + \
                                   f" - Apply a {TextManager.PurpieText('Sponge')}. Poor visibility increases the chance of a {TextManager.WarningText('Skill Failure')}.\n"

            # Diagnosis
            if not self.IsUltrasoundUsed or (not self.IsLabKitUsed and not self.IsUltrasoundUsed):
                lab_kit_text = ""
                if not self.IsLabKitUsed:
                    lab_kit_text = f"or {TextManager.PurpieText('Lab Kit')}"
                self.TrainEText += TextManager.WarningText("Diagnosis") + \
                                   f" - You can use the {TextManager.PurpieText('Ultrasound')} {lab_kit_text} to diagnose the illness\n"

            # Losing Blood
            if self.BleedingLevel > 0:
                if self.BleedingLevel < 4:
                    bleeding_text = TextManager.WarningText("Losing Blood")
                else:
                    bleeding_text = TextManager.ErrorText("Losing Blood very quickly")

                if self.Incisions > 0:
                    apply_text = f"{TextManager.PurpieText('Clamps')} to reduce {TextManager.WarningText('Bleeding')} during surgery."
                else:
                    apply_text = f"{TextManager.PurpieText('Stitches')} to reduce {TextManager.WarningText('Bleeding')}."

                self.TrainEText += bleeding_text + f" - Apply {apply_text}\n"

            # Shattered Bone
            if self.ShatteredBoneCount > 0 and self.IsUltrasoundUsed:
                scalpel_text = ""
                if not self.IsFixable and not self.IsPatientFixed:
                    scalpel_text = TextManager.PurpieText("Scalpel")

                self.TrainEText += TextManager.WarningText("Shattered Bone") + \
                                   f" - Apply {TextManager.PurpieText('Pins')}. You must put the patient to sleep with {TextManager.PurpieText('Anesthetic')} and " \
                                   f"{'make an incision with a ' + scalpel_text if scalpel_text else ''} before you can apply pins.\n"

            # Broken Bone
            if self.BrokenBoneCount > 0 and self.IsUltrasoundUsed:
                self.TrainEText += TextManager.WarningText("Broken Bone") + \
                                   f" - Apply a {TextManager.PurpieText('Splint')}.\n"

            # Fever
            if self.Fever > 0:
                fever_text = TextManager.WarningText("Fever") if self.Fever < 0.5 else TextManager.ErrorText("High Fever")
                antibiotics_text = ("Apply" if self.IsLabKitUsed else
                                    f"Diagnose the {TextManager.WarningText('Infection')} With a {TextManager.PurpieText('Lab Kit')} then apply")

                self.TrainEText += fever_text + \
                                   f" - {antibiotics_text} {TextManager.PurpieText('Antibiotics')} to bring down {TextManager.WarningText('Temp')}\n"

            # Antibiotics
            if self.Temp > 98.8 and self.Fever > 0 and self.IsLabKitUsed:
                self.TrainEText += TextManager.PositiveText("Antibiotics") + \
                                   f" - Apply {TextManager.PurpieText('Antibiotics')} to prevent any infection. If all else fails, give antibiotics.\n"

            # Pulse
            if self.Pulse < 11:
                self.TrainEText += TextManager.ErrorText("Extremely Weak Pulse") + \
                                   f" - You can increase the {TextManager.WarningText('Pulse')} with a {TextManager.PurpieText('Blood Transfusion')}."

            # Coming To
            if 0 < self.SleepLevel < 3 and self.Incisions > 0:
                self.TrainEText += TextManager.WarningText("Coming To") + \
                                   f" - Your patient is about to wake up but you still have some {TextManager.WarningText('Incisions')}. Use {TextManager.PurpieText('Anesthetic')} to keep them unconscious until you have closed the wound.\n"


    def SetCurrentPatientEmbed(self, embed : discord.Embed) -> str:  # The surgery UI
        if self.IsSurgeryEnded:
            embed.title = "Train-E" if self.TrainE else "Surg-E"
            embed.description = f"## {self.EndText}\n\n"
            embed.description += TextManager.AddFeild(value=f"**Time Elapsed:**\n{round(time.time() - self.StartTime)} Seconds\n", inline=False)
            ...
            embed.description += (TextManager.WarningText(self.SpecialConditionText) + "\n" if self.SpecialCondition != "None" and self.SpecialConditionVisibility else "")
            embed.description += f"{TextManager.BoldText(self.ScanText)}\n"
            ...
            if self.BoneText != "":
                embed.description += TextManager.AddFeild(value=f"{self.BoneText}", inline=True)
            if self.PatientText != "":
                embed.description += TextManager.AddFeild(value=f"{self.PatientText}", inline=False)
            if self.BleedingText != "":
                embed.description += TextManager.AddFeild(value=f"{self.BleedingText}", inline=False)
            if self.FeverText != "":
                embed.description += TextManager.AddFeild(value=f"{self.FeverText}", inline=False)
            if self.ToolText != "":
                embed.description += TextManager.AddFeild(value=f"{TextManager.SoftText(self.ToolText)}", inline=False)
            if self.NurseText != "":
                embed.description += TextManager.AddFeild(value=self.NurseText, inline=False)
            if self.HeartText != "":
                embed.description += TextManager.AddFeild(value=f"{self.HeartText}", inline=False)
            if self.TrainE:
...
class PatientStatus:
    ...
    def GetPatientState(patientState: Enum) -> str:
        match patientState.value:
            case 0:
                return TextManager.ErrorText('Heart Stopped!')
            case 1:
                return TextManager.ErrorText('Awake')
            case 2:
                return TextManager.WarningText('Coming To')
            case 3:
                return TextManager.PositiveText('Unconscious')
            case 4:
                return TextManager.ErrorText('Near Coma')  # Train-E Exclusive
class FileManager:
    @staticmethod
...
class ToolIcon(Enum):
    SurgicalAntibiotics = "<:SurgicalAntibiotics:1401490057482862613>"
    SurgicalAntiseptic = "<:SurgicalAntiseptic:1401490067721162843>"
    SurgicalAnesthetic = "<:SurgicalAnesthetic:1401490046993039442>"
    SurgicalClamp = "<:SurgicalClamp:1401490077854601298>" 
    SurgicalDefib = "<:SurgicalDefibrillator:1401490086562107403>"
    SurgicalPins = "<:SurgicalPins:1401490127611498506>"
    SurgicalLoveMallet = "<:SurgicalLoveMallet:1401490115678703636>"
    SurgicalSponge = "<:SurgicalSponge:1401490165293387826>"
    SurgicalScalpel = "<:SurgicalScalpel:1401490138583928933>"
    SurgicalStitches = "<:SurgicalStitches:1401490175942463609>"
    SurgicalSplint = "<:SurgicalSplint:1401490148067250186>"
    SurgicalUltrasound = "<:SurgicalUltrasound:1401490198545698836>"
    SurgicalTransfusion = "<:SurgicalTransfusion:1401490185824505948>"
    SurgicalLabKit = "<:SurgicalLabKit:1401490098591236126>"
    FixIt = "<:FixIt:1401490037006401536>"
    EmptySurgeryTray = "<:EmptySurgeryTray:1401490009810538608>"
...
class SurgeryView(View): 
    ...
    def ToolCallback(self, toolType):
        async def callback(interaction: discord.Interaction):
            ...
            embed.set_footer(text="Zoom Surg | The Best Surgery Simulator")
            await interaction.response.edit_message(embed=embed, view=self)
